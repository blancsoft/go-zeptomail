...
type (
	...
	BaseEmailOption struct {
		...
		//You can enable or disable email click tracking here. You can also
		//enable email click tracking in your Mail Agent under Email Tracking
		//section. Note: The API setting will override the Mail Agent settings
		//in your ZeptoMail account.
		//Allowed value: True - Enable email click tracking; False - Disable email click tracking.
		TrackClicks bool `json:"track_clicks"`

		//You can enable or disable email open tracking. You can also enable
		//email open tracking in your Mail Agent under Email Tracking section.
		//Note: The API setting will override the Mail Agent settings in your
		//ZeptoMail account. Allowed value: True - Enable email open tracking;
		...
		MimeHeaders map[string]any `json:"mime_headers"`

		//The attachments you want to add to your transactional emails.
		//Visit [https://www.zoho.com/zeptomail/help/file-cache.html#alink-un-sup-for]
		//to view the list of unsupported formats.
		...
		//Allowed value - Base64 content or file_cache_key or both.
		//Base64 content includes mime_type, content, and cid parameters.
		//
		//cid String
		//Content ID used by HTML body for content lookup. Each content requires a separate cid.
		//Allowed value - Base64 content or file_cache_key or both.
		InlineImages []any `json:"inline_images"`
	}

	BaseSendEmail struct {
		//Allowed value - A valid sender email address with "address"
		//and "name" key-value pairs
		...
		//You can use merge tags to replace the placeholders with multiple
		//values for different recipients. The merge tags of the mail to
		//be sent. Merge Tags can be set in HTML body, text body, subject
		//and client reference. Merge tags are tags that are set in the
		//string. These tags will be replaced by the value provided in this
		//param for that tag. In the case of single email API, the merge tag
		//is common for all recipients. Whereas in the case of batch emails,
		//the merge tag is specific to each recipient.
		MergeInfo map[string]any `json:"merge_info" validate:"required"`
	}
	...
	SendTemplatedEmailReq struct {
		...
		//Unique key identifier for your template. template_alias is the
		//alias name given to the template key. It can be used in the place
		//of template_key.
		TemplateKey string `json:"template_key" validate:"required"`

		//The email address to which bounced emails will be sent.
		//Allowed value - A valid bounce email address as configured in
		//your Mail Agent.
	...
	// SendBatchEmailTo is the object for send batch recipient emails
	SendBatchEmailTo struct {
		// A valid recipient email address with "address" and "name" key-value pairs.
		EmailAddress EmailAddress `json:"email_address" validate:"required"`

		//You can use merge tags to replace the placeholders with multiple values
		//for different recipients. The merge tags of the mail to be sent.
		//Merge Tags can be set in HTML body, text body, subject and client reference.
		//Merge tags are tags that are set in the string. These tags will be replaced
		//by the value provided in this param for that tag. In the case of single
		//email API, the merge tag is common for all recipients. Whereas in the case
		//of batch emails, the merge tag is specific to each recipient.
		MergeInfo map[string]any `json:"merge_info" validate:"required"`
	}
	...
	EmailAddress struct {
		...
		//Allowed value - A valid email address containing a domain that is verified
		//in your Mail Agent.
	...
	SendBatchTemplatedEmailReq struct {
		...
		*/
		...
		From EmailAddress `json:"from" validate:"required"`
		// Allowed value - JSON object of email_address.
		To []SendBatchEmailTo `json:"to" validate:"required"`
		// Allowed value - JSON object of reply_to email addresses.
		ReplyTo EmailAddress `json:"reply_to"`
		/*
	...
	AddEmailTemplateReq struct {
		...
		//Unique alias for the template. It can be used instead of
		//the template key. It can be obtained from the Edit template
		//section in your ZeptoMail account.
	...
	AddEmailTemplateRes struct {
		Data struct {
			...
			TemplateSize    int            `json:"template_size"` // present in docs but not included in response
			CreatedTime     string         `json:"created_time"`
			ModifiedTime    string         `json:"modified_time"`
			Subject         string         `json:"subject"`
			HtmlBody        string         `json:"htmlbody"`
			TextBody        string         `json:"textbody"`
			SampleMergeInfo map[string]any `json:"sample_merge_info"`
		} `json:"data"`
		Message string         `json:"message"`
		Error   *ErrorResponse `json:"error"`
	}
	...
	// SendBatchHTMLEmailRes is the SendHTMLEmail() response object
